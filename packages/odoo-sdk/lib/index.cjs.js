...
const odooConnector = (options) => {
    ...
    if (options.ofetch) {
        ...
        query = async (metadata, params) => {
            ...
            return options.ofetch('/api/odoo/query', { method: 'POST', body: [metadata, params], cache: 'no-cache', key: cacheKey, ...options.ofetchOptions });
        };
        queryNoCache = async (metadata, params) => {
            const cacheKey = metadata.cacheKey || hash__default["default"]({ ...metadata, ...params });
            return options.ofetch('/api/odoo/query-no-cache', { method: 'POST', body: [metadata, params], cache: 'no-cache', key: cacheKey, ...options.ofetchOptions });
        };
    }
